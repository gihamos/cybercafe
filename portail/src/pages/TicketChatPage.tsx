...
/** Chat en mode ticket (connexion anonyme par code) — même principe que le chat
 * compte, mais le fil est rattaché à la session en cours plutôt qu'à un compte, et
 * effacé à la fin de la session (sauf conservation par l'opérateur), voir
 * server/services/chat_service.py::purger_conversation_session. */
export default function TicketChatPage() {
  ...
  const charger = useCallback(async () => {
    try {
      const data = await api.get<MessageChat[]>("/portail/ticket/chat");
      setMessages(data);
    } catch {
  ...
  async function envoyer(e: FormEvent) {
    ...
    setSending(true);
    try {
      const msg = await api.post<MessageChat>("/portail/ticket/chat", { message: texte.trim() });
  ...
  return (
    <>
      ...
      <p className="muted" style={{ fontSize: 12.5, marginTop: -8 }}>
        Cette conversation est propre à votre session en cours — elle n'est pas conservée après votre déconnexion.
      </p>
