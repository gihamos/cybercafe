...
from params import CARTE_API_BASE, CARTE_API_KEY, PAYMENT_RETURN_URL, PAYMENT_CANCEL_URL
from services.payment_gateway.base import PaymentGateway, PaymentOrder


class CarteEnLigneGateway(PaymentGateway):
    """Paiement par carte bancaire à distance (portail WiFi) via une passerelle de
    ...
    Réutilise CARTE_API_BASE / CARTE_API_KEY (params.py), comme la validation carte en
    caisse."""
    ...
    def creer_commande(self, montant: float, devise: str, reference: str, description: str) -> PaymentOrder:
        response = httpx.post(
            f"{CARTE_API_BASE}/checkout_sessions",
            json={
    ...
    def capturer_commande(self, order_id: str) -> PaymentOrder:
        response = httpx.post(
            f"{CARTE_API_BASE}/checkout_sessions/{order_id}/capture",
            headers=self._headers(),
            timeout=20,
        )
        response.raise_for_status()
